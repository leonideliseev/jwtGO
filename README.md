...
Приложение берёт данные для запука из окружения проекта.
...
**ATTENTION**: Поля `PG_PASSWORD`, `PG_USER`, `PG_DATABASE`, `APP_PORT` вынесены в `docker-compose.yml` как ссылки `${}`, поэтому если убрать их из `.env`, то надо либо передавать их другим способ, либо указать конкретное значение.
